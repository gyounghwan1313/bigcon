---
title: "빅콘테스트 혁신아이디어 분야 데이터 분석 코드"
author: "팀명 : red snappers"
output:
  html_document: 
    theme: cerulean
    highlight: zenburn
...
# __신한카드 내국인 카드 소비 데이터 분석(유통업, 요식업소, 음료식품)__ 


## __분석 1 : 월별 카드 소비 금액__ *(보고서 ppt p.18, appendix ppt p.5)*
...
```
__ __
### __[table_monthly_USE_AMT] : 월별 카드 사용 금액의 합계__
...
```
### __[table_monthly_USE_RATE] : 전체 카드 사용량 대비 각 카테고리의 카드 사용 비율을 구함 (단위 %)__
```{r}
...
```
### __그래프 : 유통업, 요식업소,음료식품 카테고리의 카드 사용량/ 사용 비율 그래프__
```{r}
...
## __분석2 :2019년의 평균 카드 소비 금액 대비 2020년 2~4월의 평균과 5월의 카드 소비 금액 / 증감률 비교__ *(보고서 ppt p.19, appendix ppt p.6)*

### __[table_USE_AMT_compare] : 2019년의 평균, 20년 2~4월의 평균, 20년 5월의 카드 소비금액 비교 테이블__
```{r}
...
### __[table_USE_AMT_compare_chage] 2019년의 평균 카드 소비 금액 대비 2020년 2~4월의 평균과 5월의 카드 소비 금액 증감률 비교 테이블__
...
table_USE_AMT_compare_chage # 분석2 결과 테이블
```

### __그래프 : 2019년의 평균 카드 소비 금액 대비 2020년 2~4월의 평균과 5월의 카드 소비 금액 증감률 그래프__
```{r}
...
## __분석3 :지역구별 2019년의 평균 카드 소비 금액 대비 2020년 2~4월의 평균과 5월의 카드 소비 금액 비교__ *(보고서 ppt p.20)*

### __[data_gu_amt]: 지역구별/월별 전체, 유통업, 요식업소,음료식품 카드 소비량 구하기__
```{r}
# 지역구별 전체 카드 소비량 구하기
...
```
__ __
### __[useamt_year_table]: 2019년 소비 평균, 2020년2~4월 소비 평균, 2020년 5월 소비량을 통해 2019년대비 2020년 카드 사용액 비교__
```{r}
# 2019년 지역별 소비 액수의 평균
...
```
__ __
### __2019년 대비 2020년2~4월 소비 증감률, 2019년 대비 2020년 5월 소비증감률 비교__
```{r}
...
# __신한카드 내국인 카드 소비 데이터 분석(여행관련 산업)__

## __분석 4: 여행관련 산업(숙박, 레저용품, 레저업소, 문화취미) 카테고리의 코로나 전후 소비의 변화 분석__ *(보고서 ppt p.8,10)*
```{r}
...
# __신한카드 외국인 카드 소비 데이터 분석(내국인과 비교)__

## __분석 5: 내국인 대비 외국인의 소비 액수의 변화 추이를 분석__ *(appendix ppt p.7)*
### __[all_1920_USE_AMT] : 나라별 내국인 대비 외국인 카드 소비 액수의 비율__ 
```{r}
...
### __그래프 : 미국, 일본, 중국, 대만, 영국, 유럽, 홍콩, 아시아, 태국 - 상위 9개만 사용해서 그래프 그리기__
```{r}
...
## __분석 6: 카테고리별 내국인 대비 외국인의 카드 소비 비율의 변화 추이를 분석__*(appendix ppt p.8)*
...
## __분석 7: 주별 유동인구 변화 추이 분석__ *(보고서 ppt p.21)*
### __[year_total_flow] : 주단위 유동인구의 합계__ 
```{r}
#날짜별 유동인구 합
flow_age<-flow_age %>%
